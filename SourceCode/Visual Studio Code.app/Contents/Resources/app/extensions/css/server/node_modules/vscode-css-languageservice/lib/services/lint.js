    ...
    var LintVisitor = (function () {
        ...
        LintVisitor.prototype.visitStylesheet = function (node) {
            ...
            var _loop_1 = function(name_1) {
                var actual = keyframes.data[name_1].names;
                var needsStandard = (actual.indexOf('@keyframes') === -1);
                if (!needsStandard && actual.length === 1) {
                    return "continue"; // only the non-vendor specific keyword is used, that's fine, no warning
                }
                ...
                if (missingVendorSpecific || needsStandard) {
                    keyframes.data[name_1].nodes.forEach(addVendorSpecificWarnings);
                }
            };
            var this_1 = this;
            for (var name_1 in keyframes.data) {
        ...
        LintVisitor.prototype.visitRuleSet = function (node) {
            ...
            declarations.getChildren().forEach(function (node) {
                if (_this.isCSSDeclaration(node)) {
                    ...
                    if (firstChar === '-') {
                        ...
                        propertiesBySuffix.add(name_2, name_2, null); // don't pass the node as we don't show errors on the standard
                    }
                }
                else {
            ...
            if (!containsUnknowns) {
                var _loop_2 = function(suffix) {
                    ...
                    if (!needsStandard && actual.length === 1) {
                        return "continue"; // only the non-vendor specific rule is used, that's fine, no warning
                    }
                    var expected = [];
        ...
        LintVisitor.prototype.visitFontFace = function (node) {
            ...
            declarations.getChildren().forEach(function (node) {
                if (_this.isCSSDeclaration(node)) {
                    var name_3 = (node.getProperty().getName().toLocaleLowerCase());
                    if (name_3 === 'src') {
                        definesSrc = true;
                    }
