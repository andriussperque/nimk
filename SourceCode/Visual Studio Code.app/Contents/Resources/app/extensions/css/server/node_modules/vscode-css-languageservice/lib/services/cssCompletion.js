(function (factory) {
    if (typeof module === 'object' && typeof module.exports === 'object') {
        ...
        define(["require", "exports", '../parser/cssNodes', '../parser/cssSymbolScope', './languageFacts', '../utils/strings', 'vscode-languageserver-types'], factory);
    }
    ...
    var CSSCompletion = (function () {
        ...
        CSSCompletion.prototype.getCompletionsForDeclarationValue = function (node, result) {
            ...
            if (entry) {
                ...
                existingValues.getEntries().forEach(function (existingValue) {
                    result.items.push({
                        label: existingValue,
                        textEdit: _this.getTextEdit(existingNode, existingValue),
                        kind: 12 /* Value */
                    });
                });
            }
                ...
                entry.values.forEach(function (value) {
                    if (languageFacts.isCommonValue(value)) {
                        result.items.push({
                            ...
                            textEdit: _this.getTextEdit(existingNode, value.name),
                            kind: 12 /* Value */
                        });
                    }
                });
        ...
        CSSCompletion.prototype.getVariableProposals = function (existingNode, result) {
            ...
            symbols.forEach(function (symbol) {
                result.items.push({
                    label: symbol.name,
                    textEdit: _this.getTextEdit(existingNode, strings.startsWith(symbol.name, '--') ? "var(" + symbol.name + ")" : symbol.name),
                    kind: 6 /* Variable */
                });
            });
            return result;
        ...
        CSSCompletion.prototype.getVariableProposalsForCSSVarFunction = function (result) {
            ...
            symbols.forEach(function (symbol) {
                result.items.push({
                    label: symbol.name,
                    textEdit: _this.getTextEdit(null, symbol.name),
                    kind: 6 /* Variable */
                });
            });
            return result;
                ...
                if (units) {
                    units.forEach(function (unit) {
                        result.items.push({
                            ...
                            textEdit: _this.getTextEdit(existingNode, currentWord + unit),
                            kind: 11 /* Unit */
                        });
                    });
                }
        ...
        CSSCompletion.prototype.getTextEdit = function (existingNode, insertText) {
            if (existingNode && existingNode.offset <= this.offset) {
                ...
                return vscode_languageserver_types_1.TextEdit.replace(vscode_languageserver_types_1.Range.create(this.textDocument.positionAt(existingNode.offset), end), insertText);
            }
            return vscode_languageserver_types_1.TextEdit.replace(this.defaultReplaceRange, insertText);
        };
        CSSCompletion.prototype.getColorProposals = function (entry, existingNode, result) {
            ...
            if (entry.restrictions.indexOf('color') !== -1) {
                ...
                colorValues.getEntries().forEach(function (color) {
                    result.items.push({
                        label: color,
                        textEdit: _this.getTextEdit(existingNode, color),
                        kind: 16 /* Color */
                    });
                });
                languageFacts.colorFunctions.forEach(function (p) {
                    result.items.push({
                        ...
                        textEdit: _this.getTextEdit(existingNode, p.func.replace(/\[?\$(\w+)\]?/g, '{{$1}}')),
                        kind: 3 /* Function */
                    });
                });
            }
        ...
        CSSCompletion.prototype.getLineProposals = function (entry, existingNode, result) {
            ...
            if (entry.restrictions.indexOf('line-width') !== -1) {
                languageFacts.lineWidthKeywords.forEach(function (lineWidth) {
                    result.items.push({
                        label: lineWidth,
                        textEdit: _this.getTextEdit(existingNode, lineWidth),
                        kind: 12 /* Value */
                    });
                });
            }
        ...
        CSSCompletion.prototype.getCompletionForTopLevel = function (result) {
            ...
            languageFacts.getAtDirectives().forEach(function (entry) {
                if (entry.browsers.count > 0) {
                    result.items.push({
                        label: entry.name,
                        textEdit: _this.getTextEdit(null, entry.name),
                        documentation: languageFacts.getEntryDescription(entry),
                        kind: 14 /* Keyword */
                    });
                }
        ...
        CSSCompletion.prototype.getCompletionsForSelector = function (ruleSet, result) {
            ...
            languageFacts.getPseudoClasses().forEach(function (entry) {
                if (entry.browsers.onCodeComplete) {
                    result.items.push({
                        label: entry.name,
                        textEdit: _this.getTextEdit(existingNode, entry.name),
                        documentation: languageFacts.getEntryDescription(entry),
                        kind: 3 /* Function */
                    });
                }
            ...
            languageFacts.html5Tags.forEach(function (entry) {
                result.items.push({
                    label: entry,
                    textEdit: _this.getTextEdit(existingNode, entry),
                    kind: 14 /* Keyword */
                });
            });
            ...
            this.styleSheet.accept(function (n) {
                if (n.type === nodes.NodeType.SimpleSelector && n.length > 0) {
                    ...
                    if (selector.charAt(0) === '.' && !visited[selector]) {
                        ...
                        result.items.push({
                            label: selector,
                            textEdit: _this.getTextEdit(existingNode, selector),
                            kind: 14 /* Keyword */
                        });
                    }
                    return false;
            ...
            allFunctions.forEach(function (functionSymbol) {
                if (functionSymbol.node instanceof nodes.FunctionDeclaration) {
                    ...
                    result.items.push({
                        ...
                        textEdit: _this.getTextEdit(existingNode, functionSymbol.name + '(' + params.map(function (p) { return '{{' + p + '}}'; }).join(', ') + ')'),
                        kind: 3 /* Function */
                    });
                }
            });
